...
ggplots <- function(resampled_points) {
  ...
  MHF_and_baselinecorr_MHF <- ggplot() +
    ...
    geom_line(data = resampled_points, aes(x = TRef, y = MHF), color = "red", size = 1) +
  ...
  Overlay <- ggplot() +
    geom_line(data = resampled_points, aes(x = TRef, y = loessTHF), color = "blue", size = 1) +
    geom_line(data = resampled_points, aes(x = TRef, y = loessRHF), color = "red", size = 1) +
    geom_line(data = resampled_points, aes(x = TRef, y = loessNRHF), color = "green", size = 1) +
    labs(title = "Rolling Means for Original and Y-Equally Spaced Sine Data",
         x = "Time",
         y = "Signal / Rolling Mean") +
    theme_minimal() 
  ...
  Smoothed_RHF <- ggplot() +
    geom_line(data = resampled_points, aes(x = TRef, y = loessRHF), color = "blue", size = 1) +
    labs(title = "Rolling Means for Original and Y-Equally Spaced Sine Data",
         x = "Time",
         y = "Signal / Rolling Mean") +
    theme_minimal()
  ...
  Smoothed_NRHF <- ggplot() +
    geom_line(data = resampled_points, aes(x = TRef, y = loessNRHF), color = "blue", size = 1) +
    labs(title = "Rolling Means for Original and Y-Equally Spaced Sine Data",
         x = "Time",
         y = "Signal / Rolling Mean") +
    theme_minimal()
