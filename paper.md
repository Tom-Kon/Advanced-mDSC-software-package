...
# Software design
The core principle guiding most of the software's design was its user friendliness, especially towards people without coding experience. This is reflected in the use of a ShinyR UI that helps users to navigate the software, upload files, run analyses, and download results. Special consideration was given to make this as intuitive as in commercial software both for uploads and downloads by using a file picker rather than asking the user to use absolute paths. Another important aspect of this has been to separate the UI for the four apps constituting the software by using the shiny.router package, keeping everything clearly separated. Clear instructions as well as mathematical explanations are given in the documentation, which is included as a pdf in the github but also within the software, once more separated into relevant parts for each app. The software was written in R since it is open-source and has a wide community with many different packages and clear documentation, thus inviting contribution. Furthermore, the code is separated into four folders for the four apps, and then separated into different files for the UI, error handling, general functions, and small specific functions for each app. This, together with common naming conventions across the software, was once more done to further invite contributions.
