        ...
        fluidRow(
          column(12, wellPanel(
            selectInput(ns("plot_choice"), "Select Plot:", 
                        choices = c("THF", "RHF","NRHF", "THF FT", "RHF FT", "THF TRIOS", "RHF based on THF TRIOS", "NRHF based on THF TRIOS"), 
                        selected = "THF"),
          ))
        ),
        fluidRow(
  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$plot <- renderPlotly({
      ...
      plot_obj <- switch(input$plot_choice,
                         ...
                         "THF TRIOS" = THFplotTRIOS(reactive_inputs$RHFdf2),
                         "RHF based on THF TRIOS" = RHFplotTRIOS(reactive_inputs$RHFdf2), 
                         "NRHF based on THF TRIOS" = NRHFplotTRIOS(reactive_inputs$RHFdf2) 
      )
