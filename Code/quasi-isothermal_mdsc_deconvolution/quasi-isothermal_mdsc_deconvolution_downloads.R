downloadExcel <- function(fileName, results, modulationsBack, period, setAmplitude, startingTemp, stepSize, isothermLength, sampling) {
  ...
  resultsFTexport <- data.frame("Step number" = resultsFT$pattern, 
                                "Temperature at that modulation" = resultsFT$TRef,
                                "Non-reversing heat flow" = resultsFT$NRHF, 
                                "Reversing heat flow" = resultsFT$RevCp)
